...
# Optionaler Fallback: nur aktiv, wenn explizit eingeschaltet
USE_PLAYWRIGHT = os.getenv("USE_PLAYWRIGHT", "0").strip() == "1"

# Optionaler GitHub-State/Archiv
GITHUB_TOKEN = os.getenv("GITHUB_TOKEN")
GH_REPO = os.getenv("GH_REPO")  # z.B. "owner/repo"
...
def ensure_playwright():
    """Installiert Playwright nur, wenn USE_PLAYWRIGHT=1 gesetzt ist."""
    global _PW_READY
    if _PW_READY:
        return
    try:
        ...
        pass
    # Versuch, Playwright in dieser Session zu installieren
...
def fetch_json_playwright(url):
    if not USE_PLAYWRIGHT:
        raise RuntimeError("Playwright-Fallback ist deaktiviert.")
    ensure_playwright()
    if not _PW_READY:
...
def fetch_results():
    try:
        ...
        print(f"requests-Pfad gescheitert: {e_req}")
        if USE_PLAYWRIGHT:
            print("Versuche Playwright-Fallback …")
            return fetch_json_playwright(API_URL)
        # Schnell wieder raus (kein teurer Browser-Fallback)
        raise
...
# ---------- State (optional GitHub-Issue oder lokale Datei) ----------
...
# ---------- Archiv (optional ins Repo schreiben) ----------
